...
function init() {
  program
    ...
    .option('--type <dir>', 'css-in-js | css-module')
    ...
    .action((genType, displayName, options) => {
      switch (genType) {
        case GEN_TYPES.page:
          ...
          break
        case GEN_TYPES.fc:
          generateFc(displayName, options.out)
          break
        default:
...
function generateFc(displayName, optionOut) {
  const componentName = format(displayName, FORMAT_TYPES.upperCamelCase)
  const outRootDir = path.resolve(process.cwd(), optionOut || '')

  const templateDir = path.resolve(__dirname, TEMPLATES_DIRS.fc)
  createFiles(templateDir, outRootDir, {
